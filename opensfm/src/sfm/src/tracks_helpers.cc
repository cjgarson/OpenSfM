...
std::unordered_map<map::ShotId, int> CountTracksPerShot(
    ...
    const std::vector<map::ShotId>& shots,
...
void AddConnections(map::TracksManager& manager,
                    const map::ShotId& shot_id,
...
void RemoveConnections(map::TracksManager& manager,
                       const map::ShotId& shot_id,
                       const std::vector<map::TrackId>& connections) {
  for (const auto& connection : connections) {
