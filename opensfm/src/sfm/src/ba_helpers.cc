...
std::unordered_set<map::Shot*> BAHelpers::DirectShotNeighbors(
  ...
  std::sort(pairs.begin(), pairs.end(),
            [](const std::pair<map::Shot*, size_t>& a,
               const std::pair<map::Shot*, size_t>& b) {
...
py::tuple BAHelpers::BundleLocal(
  ...
  report["wall_times"]["run"] =
      std::chrono::duration_cast<std::chrono::microseconds>(timer_run - timer_setup)
          .count() / 1e6;
  report["wall_times"]["teardown"] =
      std::chrono::duration_cast<std::chrono::microseconds>(timer_teardown - timer_run)
          .count() / 1e6;
